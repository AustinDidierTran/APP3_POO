   ...
   for ( i = 0; i < NBRE_PLATS_MAX; i++ ) 
   {
       ...
       else
       {
          if (TRACE_Menu)
          {
              ...
              cout << "Plat No " << i <<" non defini " << endl;
          }
       }
   }
   return;
...
bool Menu::positionne_code(int code_a_trouver)      ///< positionne le courant au code
{
   ...
   // positionne à la premiere occurence du code
   i = 0;
   trouve = false;
...
PlatAuMenu * Menu::get_courant ()              ///< Obtenir une référence au plat courant
{
   if (TRACE_Menu)
